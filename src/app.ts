...
const el = {
  ...
  whoNote: element<HTMLElement>('whoNote'),
  skipFace: element<HTMLButtonElement>('skipFace'),
 ...
 * stage.
 */
function cannotReadYet(names: string[], named: number): string {
  if (named === 0) {
...
async function practise(): Promise<void> {
  ...
  toast(
    `${files.length} drawn shapes queued — a shape on a coloured background, so naming one teaches two things. ` +
