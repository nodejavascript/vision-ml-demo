...
function startTraining(): void {
  ...
  if (labelled.length < 4 || classes.length < 2) {
    setStatus(
      `It needs at least two things to tell apart with two images each — right now it has ${classes.length} class${classes.length === 1 ? '' : 'es'} ` +
        `and ${labelled.length} named image${labelled.length === 1 ? '' : 's'}. Name some below.`,
      'warn',
    );
    return;
  }
