...
import type { EpochMetric, HostMessage, HostRequest, TrainPayload, TrainSample } from './types.js';
...
export class VisionTrainer {
  ...
  private net: Cnn | null = null;
  private payload: TrainPayload | null = null;
  private rng: () => number = Math.random;
  private plan: TrainSample[] = [];
  private watch: TrainSample[] = [];
  private history: EpochMetric[] = [];
  private epoch = 0;
  ...
  handle(request: HostRequest): void {
    switch (request.type) {
      case 'train':
  ...
  private schedule(): void {
    if (!this.channel) {
      ...
      this.channel.port1.onmessage = (): void => this.pump();
    }
   ...
   */
  private split(payload: TrainPayload): { train: TrainSample[]; watch: TrainSample[] } {
    const groups = new Map<number, TrainSample[]>();
    for (const sample of payload.samples) {
      const list = groups.get(sample.classIndex);
      if (list) list.push(sample);
      else groups.set(sample.classIndex, [sample]);
    }

    const train: TrainSample[] = [];
    const watch: TrainSample[] = [];
    for (const group of groups.values()) {
      ...
      for (let i = 0; i < held; i++) watch.push(group[i]);
      for (let i = held; i < group.length; i++) train.push(group[i]);
    }
    return { train, watch };
  }

  /** One epoch. Returns false once training has ended, by finishing or by stopping. */
  private runOneEpoch(): boolean {
    const net = this.net;
    const payload = this.payload;
    if (!net || !payload) return false;
    ...
    this.epoch += 1;
    const metric: EpochMetric = { epoch: this.epoch, loss, trainAccuracy: accuracy, valAccuracy, valLoss };
  ...
  private finish(kind: 'done' | 'stopped'): void {
    const net = this.net;
    const payload = this.payload;
    if (!net || !payload) return;
    ...
    const confusion: number[][] = Array.from({ length: size }, () => new Array<number>(size).fill(0));
    for (const sample of all) {
      const probs = net.predict(sample.pixels);
      let best = 0;
      for (let j = 1; j < size; j++) if (probs[j] > probs[best]) best = j;
      if (sample.classIndex < size && best < size) confusion[sample.classIndex][best] += 1;
    }
    ...
    const metric: EpochMetric = this.last ?? { epoch: this.epoch, loss: 0, trainAccuracy: 0, valAccuracy: 0, valLoss: 0 };
