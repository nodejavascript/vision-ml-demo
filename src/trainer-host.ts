...
export class VisionTrainer {
  ...
  private split(payload: TrainPayload): { train: TrainSample[]; watch: TrainSample[] } {
    ...
    for (let c = 0; c < payload.classes.length; c++) {
      if (train.some((s) => s.targets.includes(c))) continue;
      const donor = watch.findIndex((s) => s.targets.includes(c));
      if (donor >= 0) train.push(watch.splice(donor, 1)[0]);
    }
    return { train, watch };
  }
