 ...
 */
import { BATCH, CONV1, CONV2, HIDDEN, SAMPLE, emptyMeta, } from './types.js';
...
function fillHe(target, fanIn, rng) {
    const scale = Math.sqrt(2 / fanIn);
    for (let i = 0; i < target.length; i++)
        target[i] = gaussian(rng) * scale;
}
/* ------------------------------------------------------------------ *
 * Pixels
 * ------------------------------------------------------------------ */
 ...
 */
export function normalise(pixels, out) {
    const n = Math.min(pixels.length, out.length);
    for (let i = 0; i < n; i++)
        out[i] = pixels[i] / 127.5 - 1;
    return out;
}
/**
 ...
 */
export function augment(pixels, size, rng) {
...
export class Cnn {
     ...
     */
    warmupSteps = 10;
    constructor(classes, size = SAMPLE, hidden = HIDDEN, seed = 1234) {
        this.size = size;
        this.q1 = size / 2;
        this.q2 = size / 4;
    ...
    /* ---------------- forward ---------------- */
    conv(input, channels, height, width, weight, bias, filters, out) {
        const w = weight.data;
    ...
    pool(input, channels, height, width, out, arg) {
        const oh = height / 2;
        const ow = width / 2;
        for (let c = 0; c < channels; c++) {
    ...
    applyGradients(scale, lr, beta1 = 0.9, beta2 = 0.999, eps = 1e-8) {
        this.step += 1;
     ...
     */
    trainEpoch(samples, lr, useAugment, rng) {
    ...
    labelAccuracy(probs) {
        let right = 0;
        for (let j = 0; j < this.classes; j++) {
            if ((probs[j] >= 0.5 ? 1 : 0) === this.target[j])
                right += 1;
        }
    ...
    meanConfidence(samples) {
        ...
        for (const sample of samples) {
            ...
            for (let j = 0; j < this.classes; j++)
                total += Math.max(probs[j], 1 - probs[j]);
        }
        return total / (samples.length * this.classes);
    }
    /* ---------------- save and load ---------------- */
    serialize(classes, history, images, perClassAccuracy, perClassCount, meanConfidence, previous) {
...
function dense(input, inputs, out, weight, bias, outputs) {
    const w = weight.data;
 ...
 */
function denseBackward(input, inputs, gOut, outputs, weight, bias, gIn) {
    for (let i = 0; i < inputs; i++)
        gIn[i] = 0;
    for (let j = 0; j < outputs; j++) {
        const g = gOut[j];
...
function poolBackward(gOut, channels, size, arg, gIn) {
    gIn.fill(0);
    const half = size / 2;
    for (let c = 0; c < channels; c++) {
 ...
 */
function convBackward(input, channels, height, width, gOut, filters, weight, bias, gIn) {
    if (gIn)
        gIn.fill(0);
    const w = weight.data;
    for (let f = 0; f < filters; f++) {
...
function sigmoid(logits, out, count) {
 ...
 * ------------------------------------------------------------------ */
const CHUNK = 8192;
export function encodeFloats(values) {
