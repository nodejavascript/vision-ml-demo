...
function grow(src, width, height, dst) {
...
function shrink(src, width, height, dst) {
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            let on = 1;
 ...
 */
function closeHoles(mask, width, height) {
...
function blobsOf(mask, width, height) {
    ...
    for (let start = 0; start < mask.length; start++) {
        ...
        stack[top++] = start;
        seen[start] = 1;
        const blob = { left: width, top: height, right: -1, bottom: -1, count: 0 };
        while (top > 0) {
...
export function detectFaces(frame) {
    ...
    for (const blob of blobsOf(mask, width, height)) {
        ...
        if (share < LEAST_AREA || share > MOST_AREA)
            continue;
        const shape = w / h;
        if (shape < NARROWEST || shape > WIDEST)
            continue;
        const fill = blob.count / boxArea;
        if (fill < LEAST_FILL || fill > MOST_FILL)
            continue;
    ...
    for (const box of found) {
        if (kept.some((other) => overlap(other, box) > 0.35))
            continue;
        kept.push(box);
        if (kept.length === MOST_FACES)
            break;
    }
