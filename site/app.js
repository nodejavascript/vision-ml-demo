...
function renderQueue() {
    ...
    if (boxes > 0)
        parts.push(`box ${state.turnIndex + 1} of ${boxes}`);
    // "picture", not "photo": the page says picture everywhere else.
    if (hasRun())
        parts.push(`picture ${runPosition()} of ${runTotal()}`);
    if (state.opening)
...
function showPractiseOffer(nothingNamed) {
    ...
    el.aside.append(` on ${PRACTISE_COUNT} shapes it draws itself — ${SHAPE_NAMES.length} kinds, several of each.`);
